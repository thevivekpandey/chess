...
def run_pipeline(
    ...
    print(f"  Pre-fill cache dir: {prefill_cache_dir or '(none — using work_dir/selfplay_games)'}")
    print("=" * 80)
    print()
    ...
    if prefill_iterations > 0 and replay_buffer.size() == 0:
        # Where prefill CSVs live. If --prefill-cache-dir was given, write/read
        # there so the data can be reused across runs. Otherwise fall back to
        # the per-run selfplay_dir (legacy behavior).
        prefill_dir = prefill_cache_dir if prefill_cache_dir else selfplay_dir
        os.makedirs(prefill_dir, exist_ok=True)

        print(f"\n{'=' * 80}")
        print(f"PRE-FILL PHASE ({prefill_iterations} iterations of self-play, no training)")
        print(f"  Prefill data dir: {prefill_dir}")
        print(f"{'=' * 80}\n")
        prefill_start = time.time()
        for prefill_iter in range(prefill_iterations):
            prefill_csv = os.path.join(prefill_dir, f"prefill_{prefill_iter:04d}.csv")
            if os.path.exists(prefill_csv):
                print(f"\n[PREFILL {prefill_iter + 1}/{prefill_iterations}] "
                      f"Loading cached {prefill_csv}")
            else:
                print(f"\n[PREFILL {prefill_iter + 1}/{prefill_iterations}] "
                      f"Generating {games_per_iteration} self-play games -> {prefill_csv}")
...
def main():
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.3,
        help="Alpha for Dirichlet noise (default: 0.3)"
    )
    parser.add_argument(
        "--dirichlet-epsilon",
        type=float,
